...
% Workflow:
%   - prompt user for peak time in seconds
%   - snap to local envelope peak near that time
%   - expand bounds using current acceptance threshold
%   - refine bounds (main burst) if enabled
%   - append candidate and rebuild working list
...
if ~isempty(app.state.calls_on)
    % suggest current selected onset as a starting hint
...
% Snap to local peak near entered time
pkWin_ms = 1.0; % +/- 1 ms search
...
% Optional: refine to main burst to trim echoes
...
% Recompute auto keep at current threshold
...
% Rebuild working list (bw mask applied by applyFilter_v7)
[app.state.calls_on, app.state.calls_off, app.state.dispToFixed] = krgui.applyFilter_v7(app.state);

% Select newly added call if it appears
